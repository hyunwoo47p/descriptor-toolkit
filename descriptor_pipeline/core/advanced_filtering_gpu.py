...
from descriptor_pipeline.core.advanced_filtering import DisjointSet
...
class NonlinearDetectionPassGPU:
    ...
    def __init__(self, config: PipelineConfig, verbose: bool = True,
        ...
        self.config = config
        self.verbose = verbose
        self.threshold = getattr(config, 'nonlinear_threshold', 0.75)
        self.w_hsic = getattr(config, 'w_hsic', 0.3)
        self.w_rdc = getattr(config, 'w_rdc', 0.7)
