...
class StatisticsAndVarianceFilter:
    ...
    def __init__(self, config: PipelineConfig, device: torch.device, verbose: bool = True):
        self.config = config
        self.device = device
    ...
    def compute(self, parquet_paths: List[str], columns: List[str]) -> Tuple[List[str], Dict, np.ndarray]:
        ...
        self._log(f"  Variance threshold: {self.config.variance_threshold}")
        self._log(f"  Max missing ratio: {self.config.max_missing_ratio}")
        self._log(f"  Min effective N: {self.config.min_effective_n}")
        self._log(f"  Using {'GPU' if self.using_gpu else 'CPU'}")
        
        # Check for cached statistics
        cache_file = Path(self.config.output_dir) / "pass1_statistics_cache.npz"
        stats_from_cache = False
        
        if cache_file.exists() and not self.config.force_recompute:
            try:
    ...
    def _compute_statistics_pyarrow(self, parquet_paths: List[str], columns: List[str]) -> Dict:
        ...
        use_percentile = self.config.use_percentile_clip
        
        # Prepare all quantile points in one list to minimize data scans
        # Always need: min(0), max(1), trim_low, trim_high
        trim_low = self.config.trim_percentiles[0] / 100.0
        trim_high = self.config.trim_percentiles[1] / 100.0
        
        # Initialize p_low and p_high
        p_low = self.config.percentile_low / 100.0 if use_percentile else 0.0
        p_high = self.config.percentile_high / 100.0 if use_percentile else 1.0
    ...
    def _filter_low_variance(self, columns: List[str], stats: Dict) -> Tuple[List[str], np.ndarray]:
        ...
        if self.config.use_robust_variance:
            norm_var = stats['normalized_variance_robust']
        else:
            norm_var = stats['normalized_variance']
        
        # Filter criteria
        variance_mask = norm_var >= self.config.variance_threshold
        missing_mask = stats['missing_ratio'] <= self.config.max_missing_ratio
        effective_n_mask = stats['n_valid'] >= self.config.min_effective_n
    ...
    def _get_config_hash(self) -> str:
        """캐시 검증을 위한 설정 해시 생성"""
        import hashlib
        config_str = f"{self.config.variance_threshold}_{self.config.max_missing_ratio}_" \
                    f"{self.config.min_effective_n}_{self.config.use_robust_variance}_" \
                    f"{self.config.trim_percentiles}"
