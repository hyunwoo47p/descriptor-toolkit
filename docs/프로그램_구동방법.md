...
### 6.1 Track 1 출력 (`cdml run`)

| 파일명 | 설명 |
|--------|------|
| `sampled_data.parquet` | Pass 0: 샘플링된 데이터 |
| `pass1_variance_filtering.json` | Pass 1: 분산 필터링 결과 |
| `pass1_columns.txt` | Pass 1: 통과한 컬럼 목록 |
| `pass1_stats.npz` | Pass 1: 통계 정보 (평균, 분산, 범위 등) |
| `pass2_spearman.json` | Pass 2: Spearman 클러스터링 결과 |
| `pass2_columns.txt` | Pass 2: 대표 descriptor 목록 |
| `pass2_spearman_matrix.npy` | Pass 2: Spearman 상관 행렬 |
| `pass3_vif.json` | Pass 3: VIF 필터링 결과 |
| `pass3_columns.txt` | Pass 3: 통과한 컬럼 목록 |
| `pass4_nonlinear.json` | Pass 4: HSIC/RDC 비선형 필터링 결과 |
| `final_descriptors.txt` | 최종 선택된 descriptor 목록 |
| `final_cluster_info.json` | 클러스터 구조 (대표 + 대안 descriptor 매핑) |
...
`example/` 및 `reference/` 폴더에 있는 예시 데이터로 전체 워크플로우를 실행한 결과입니다.

| 파일 | 샘플 수 | 설명 |
|------|---------|------|
| `example/Ligand_U_leaching_sample.csv` | 77 | Track 1 입력 데이터 (SMILES + pLeach 포함) |
| `reference/train_samples_61.csv` | 61 | Track 2 학습 데이터 (863개 descriptor 포함) |
| `reference/test_samples_16.csv` | 16 | Track 2 테스트 데이터 (외부 검증용) |

### 7.2 Track 1: Descriptor 추출 및 필터링

```bash
# 통합 파이프라인 실행 (SMILES → Descriptor 계산 → 필터링)
cdml process-all \
...
> **참고**: 이미 descriptor가 계산된 parquet 파일이 있는 경우 `cdml run --parquet-glob "*.parquet"` 명령어를 사용할 수 있습니다.
...
| Pass 3 | 클러스터 대표 | ~80 | VIF 다중공선성 필터링 |
| Pass 4 | ~80 descriptors | 71 | HSIC/RDC 비선형 필터링 |
...
├── pass1_variance_filtering.json  # Pass 1: 분산 필터링 결과
├── pass1_columns.txt              # Pass 1: 통과한 컬럼 목록
├── pass1_stats.npz                # Pass 1: 통계 정보
├── pass2_spearman.json            # Pass 2: Spearman 클러스터링 결과
├── pass2_columns.txt              # Pass 2: 대표 descriptor 목록
├── pass2_spearman_matrix.npy      # Pass 2: 상관 행렬
├── pass3_vif.json                 # Pass 3: VIF 필터링 결과
├── pass3_columns.txt              # Pass 3: 통과한 컬럼 목록
├── pass4_nonlinear.json           # Pass 4: 비선형 필터링 결과
├── final_descriptors.txt          # 최종 71개 descriptor 목록
└── final_cluster_info.json        # 클러스터 구조 (대표 + 대안 매핑)
```
