...
class SpearmanComputerGPU:
    ...
    def __init__(self, filtering_cfg: FilteringConfig, io_cfg: IOConfig, system_cfg: SystemConfig, 
                 seed_mgr: SeedManager, device: torch.device, use_float32: bool = True):
        ...
        # Use default 40GB for GPU memory management
        max_gpu_memory = 40.0
...
class HSICComputerGPU:
    ...
    def __init__(self, filtering_cfg: FilteringConfig, io_cfg: IOConfig, system_cfg: SystemConfig, seed_mgr: SeedManager,
    ...
    def compute(self, parquet_paths: List[str], columns: List[str],
        ...
        start_time = time.time()
        p = len(columns)
        d = self.filtering_cfg.hsic_D
        r = self.filtering_cfg.r
...
class RDCComputerGPU:
    ...
    def compute(self, parquet_paths: List[str], columns: List[str],
        ...
        start_time = time.time()
        p = len(columns)
        d = self.filtering_cfg.rdc_d
        r = self.filtering_cfg.r
        n_rdc = self.filtering_cfg.rdc_seeds
